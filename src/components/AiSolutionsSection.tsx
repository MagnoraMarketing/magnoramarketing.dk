  ...
  const solutions = [
    {
      ...
      badge: t('aiSolutionsSection.solutions.widget.badge'),
      title: t('aiSolutionsSection.solutions.widget.title'),
      description: t('aiSolutionsSection.solutions.widget.description'),
      features: t('aiSolutionsSection.solutions.widget.features', { returnObjects: true }) as string[],
      cta: { text: t('aiSolutionsSection.solutions.widget.ctaText'), href: '/digital/ai-widget' },
      accent: 'from-blue-500 to-indigo-600',
    },
    {
      icon: <Mic size={28} className="text-indigo-500" />,
      badge: t('aiSolutionsSection.solutions.voice.badge'),
      title: t('aiSolutionsSection.solutions.voice.title'),
      description: t('aiSolutionsSection.solutions.voice.description'),
      features: t('aiSolutionsSection.solutions.voice.features', { returnObjects: true }) as string[],
      cta: { text: t('aiSolutionsSection.solutions.voice.ctaText'), href: '/digital/ai-reception' },
    ...
    <section className={`section ${bg}`}>
      <div className="container">
        <div className="text-center mb-14">
          <span className="section-label">{t('aiSolutionsSection.label')}</span>
          <h2 className={`mb-4 ${headingColor}`}>{t('aiSolutionsSection.heading')}</h2>
          <p className={`max-w-2xl mx-auto ${subColor}`}>
            {t('aiSolutionsSection.subtitle')}
          </p>
        </div>
        ...
        <div className={`rounded-2xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center ${variant === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-blue-600 text-white'}`}>
          {[
            { value: '24/7', label: t('aiSolutionsSection.stats.availability') },
            { value: '< 1 sek', label: t('aiSolutionsSection.stats.responseTime') },
            { value: '+40%', label: t('aiSolutionsSection.stats.moreLeads') },
            { value: '3-5 dage', label: t('aiSolutionsSection.stats.timeToLive') },
          ].map((stat, i) => (
            <div key={i}>
